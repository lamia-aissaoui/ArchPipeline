...
| 6 | Model checkpoints / checkpoint identifiers | R1-C7, R1-C9 | `checkpoints/` (HuggingFace Hub links) | ❌ **not yet uploaded anywhere** — must upload to HF Hub and update README links |
...
| 16 | ADD_COMPONENT failure log, all 20 instances, per-instance category | R2-C6 | `results/ablation_significance_analysis/add_component_all_20_cases.json` | ⚠️ EM scores present and verified (8/20 = 0.40, matches the precision cited in the letter), but **failure-category labels** (context inconsistency, structural displacement, etc.) are not yet explicit fields in the file — still to be added |
| 17 | **Manual architectural review, 20 instances, 5 judgment categories** | R2-C5 | `results/ablation_significance_analysis/manual_review_20instances.json` | ❌ **NOT YET PRODUCED.** File contains only the raw extraction (request/reference/predictions) — no human judgments yet. See `MANUAL_REVIEW_GUIDE.md` and `manual_review_20instances_TO_FILL.csv` for the ready-to-fill template. **This remains the single most important missing artifact.** |
...
1. **Produce the manual architectural review judgments** (#17) — requires real human review, cannot be generated by code or by an AI assistant on the authors' behalf.
2. **Add failure-category labels to `add_component_all_20_cases.json`** (#16), following the same categorization principle (context inconsistency, structural displacement, etc.) described in the letter for Table 8.
3. **Upload checkpoints** (#6) to HuggingFace Hub and update the links in `README.md`.
4. **Do not include** `resultats_baselines_vs_pipeline.json` or any other superseded few-shot run in the repository (#12b).
5. **Do not include** any standalone "07_Paired_Comparison_20_Instances.ipynb" file — the analysis lives in notebook 05 (#13b).
6. **Decouple notebooks from Google Drive**: replace hardcoded `/content/drive/...` paths with a configurable `BASE_DIR`/`EVO_DIR` variable, and document the expected folder layout in the README.
7. **Pin dependency versions**: run `pip freeze` in the working Colab session and commit it alongside `requirements.txt`.
